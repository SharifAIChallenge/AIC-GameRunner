...
def schedule_sending_response(run):
    if run.response_queue_reference_id is None and run.response == Run.SENDING:
        run.response_queue_reference_id = execute_run.delay(run.pk)
        run.save()
...
def periodic_check_for_sending_responses():
    for unknown_run in Run.objects.filter(response_queue_reference_id__isnull=True, response=Run.SENDING):
        schedule_sending_response(unknown_run)